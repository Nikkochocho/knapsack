...
def search(start: str, goal: str, graph: dict,
           initial_speed=None, time_limit=None,
           tp=10, ng=20, tc=0.8, tm=0.1, ig=0.2,
           **_) -> SearchResult:

    speed = initial_speed if initial_speed is not None else INITIAL_SPEED

    initial_path, final_path, initial_value, final_value = GA(
        start=start, goal=goal,
        speed=speed, time_limit=time_limit or 0.0,
    ...
    return SearchResult(
        ...
        profit=      round((final_value - initial_value) / time_limit * 100, 2) if time_limit and time_limit > 0 else round(final_value - initial_value, 2),
    )